...
- `src/edit.ts` (`applyCanvasEdit`) — real "Canvas → Code": takes a
  ...
  `ComponentInsert` (including string and typed number/boolean/expression props, optional direct text, and dot-separated member-expression tags such as `UI.Button`), `Delete`, `Duplicate`, `TextChange`, and `PropRemove`, matching the shape
