...
- `src/edit.ts` (`applyCanvasEdit`) — real "Canvas → Code": takes a
  structured `CanvasEdit` (`StyleChange`, `StyleRemove`, `StyleClear`, `PropChange`, `Reparent`, `DeleteMany`, `DuplicateMany`, or
...
- `src/*.test.ts` — 105 real tests (Node's built-in `node:test` runner, no
