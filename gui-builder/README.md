...
- `src/edit.ts` (`applyCanvasEdit`) — real "Canvas → Code": takes a
  structured `CanvasEdit` (`StyleChange`, `StyleRemove`, `StyleClear`, `PropChange`, `Reparent`, `DeleteMany`, `DuplicateMany`, `ReorderMany`, or
...
- `src/*.test.ts` — 107 real tests (Node's built-in `node:test` runner, no
