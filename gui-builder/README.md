...
- `src/edit.ts` (`applyCanvasEdit`) — real "Canvas → Code": takes a
  structured `CanvasEdit` (`StyleChange`, `StyleRemove`, `StyleClear`, `StyleClearMany`, `PropChange`, `TagChangeMany`, `Reparent`, `DeleteMany`, `DuplicateMany`, `ReorderMany`, or
...
- `src/*.test.ts` — 111 real tests (Node's built-in `node:test` runner, no
