 ...
 * it was at task #271: every hunk block also fetches its real per-line
 * detail (`git_hunk_lines`) and renders each change line with a checkbox.
 * "Stage selected (n)" applies exactly those lines via `git_stage_lines`/
 * `git_unstage_lines` -- the per-line refinement of the whole-hunk buttons,
 * kept to the same one-hunk-at-a-time, re-fetch-between discipline the
 * backend's own methods document (selecting every change line equals the
 * whole-hunk button; selecting nothing is disabled).
 *
 ...
 * silently omitting it: no stash-during-merge interplay, no branch
 * delete/rename, no merging via drag-and-drop.
 */
export default function GitPanel({ root }: GitPanelProps): React.ReactElement {
  ...
  const applySelectedLines = useCallback(
    ...
    [root, refresh, refreshHunks, selectedLines]
  );
