...
- **GUI Builder**: desktop-only visual JSX/TSX design mode with a structural component tree,
  ...
  visual undo/redo controls with Ctrl/Cmd+Z, Ctrl/Cmd+Shift+Z, Ctrl/Cmd+Y, style/prop clipboard shortcuts, dedicated Ctrl/Cmd+Alt+B/P subtree clipboard shortcuts, and versioned system-clipboard interoperability with in-memory fallback, unsaved-buffer-aware parsing/preview with stale-refresh protection, keyboard-accessible structure-tree selection and expansion, safe JSX tag renaming, multi-selection tag renaming, selected-element wrapping/unwrapping and multi-selection grouping, sibling move-up/move-down controls, multi-selection sibling reordering, child and sibling component insertion, typed component-scaffold and playground-companion creation with safe project-root file creation, component-palette child/sibling placement control, component usage counts and deprecated-export replacement hints, component palette TypeScript prop API hints including relative imported interfaces, API-aware component insertion with typed required/optional prop controls and an insert-without-props escape hatch, component usage inspection with line-level Editor navigation, collapsible and searchable structure-tree navigation, source-location reveal links into the Editor, exact selected-element JSX source copy, versioned same-file JSX subtree copy/paste with child/sibling placement across multi-selection, selected-element accessibility auditing for alt text, accessible names, target size, and contrast with copyable live reports, deterministic accessibility auto-fix suggestions for selected elements, image-as-background asset composition, sanitized inline SVG markup copy, one-click Stack/Row/Grid/Center auto-layout presets, prop-to-token binding for string props and selected style properties, tiered primitive/semantic/component token analysis with alias references, project-scoped named preview themes built from CSS tokens, project-wide design-token usage counts with line-level source navigation, project-wide image/font asset usage counts with line-level source navigation, unsaved-buffer-aware token/asset usage indexing, unsaved-buffer-aware component palette usage indexing, rendered geometry/computed-style inspection with full box-model/flex/grid/overflow details and a removable visual box-model overlay, CSS snapshot copy and focus/hover/active-state preview controls, custom responsive viewport sizing with project-scoped named presets, project-scoped named responsive breakpoints in the comparison matrix, live selected-element geometry/style diffs between adjacent breakpoint tiles, synchronized canvas selection highlights, expanded guided layout/position/effects style controls, preview-only prop/style variants with saved presets, portable interaction/data-state preset clipboard sharing, direct text editing and batch text replacement, literal/expression JSX prop copy/paste across selected nodes with literal-only cross-file fallback, atomic multi-property style/prop clipboard paste with literal-only cross-file fallback, typed expression props, existing-prop/style quick pickers, prop removal, style-property removal, one-click plain inline-style clearing with dynamic-style safety, atomic batch prop/style changes with dynamic-style protection, reusable per-file interaction-state presets, multi-selection subtree deletion with overlap and root safety guards, multi-selection subtree duplication with overlap and root safety guards, configurable inserted-component props and text, metadata-aware font asset discovery with @font-face snippet copy, editable and creatable CSS tokens, and two-way AST edits. The standalone `gui-builder/` package has 199 focused and end-to-end tests and is built separately from the
  Rust workspace.
...
- GUI Builder font assets expose inferred family, weight, and italic metadata, can apply their derived family name to one or many selected elements, and retain the copyable metadata-aware `@font-face` snippet action.
