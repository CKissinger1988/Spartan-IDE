...
- **GUI Builder**: desktop-only visual JSX/TSX design mode with a structural component tree,
  ...
  visual undo/redo controls, unsaved-buffer-aware parsing/preview, collapsible structure-tree navigation, custom responsive viewport sizing, synchronized canvas selection highlights, preview-only prop/style variants with saved presets, direct text editing, typed expression props, existing-prop/style quick pickers, prop removal, style-property removal, configurable inserted-component props and text, font asset discovery, editable CSS tokens, and two-way AST edits. The standalone `gui-builder/` package has 89 focused and
