//! Spike 0.3 (partial): the structured-output fallback parser from section 3.4.
//! Real models are not reachable in this sandbox (no Ollama/GPU, no live API key
//! in this tool), so this validates the highest-risk COMPONENT of that spike --
//! the streaming parser's correctness and, just as importantly, its failure
//! behavior -- against synthetic token streams shaped like real model output,
//! including deliberately adversarial/malformed ones.
