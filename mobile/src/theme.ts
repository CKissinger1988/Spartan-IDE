...
// second, independently-invented palette. `accent`/`accentDim` are real,
// deliberately re-picked slightly deeper shades of their dark-theme
// counterparts (not the identical hex) so accent-colored text/icons keep
// real, legible contrast against a light background instead of washing
// out -- the identical reasoning `desktop/src/theme.css`'s own doc
// comment already gives. `green`/`amber`/`red`/`teal` are kept at their
// real dark-theme hues -- these are always used as solid badge/dot fills
// paired with fixed white text, not as text-on-background, so they read
// correctly on a light background unchanged.
export const C_LIGHT = {
  ...
  accent: '#A3341F',
  accentDim: '#7A2717',
  accentBg: 'rgba(163,52,31,0.10)',
  accentBorder: 'rgba(163,52,31,0.35)',
  green: '#4E9E72',
  greenBg: 'rgba(78,158,114,0.13)',
  amber: '#C99A3D',
  amberBg: 'rgba(201,154,61,0.13)',
