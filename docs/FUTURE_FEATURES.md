...
| Feature | Priority | Notes / grounding |
|---|---|---|
| ~~Visual style editing (color/spacing/typography controls)~~ | ✅ done | Shipped (task #277). The Design screen's `StyleChange` form is no longer two raw text fields: a curated 35-property dropdown grouped into Color/Typography/Spacing/Layout/Border/Effects, each paired with a real widget matching its value space — a native color swatch **plus** a text field (so `var(--token)`/`transparent`/`rgba(...)` still round-trip; the swatch only agrees when the value is genuinely a hex), a number+unit pair for lengths (px/rem/em/%/vh/vw/unitless, with an unparseable keyword like `auto` kept editable rather than silently dropped), and enum `<select>`s for `fontWeight`/`display`/`flexDirection`/`justifyContent`/`alignItems`/`textAlign`/`borderStyle`. Picking a property seeds the control from the selected node's **real current** style entry (an `expression` entry like `color: C.text` deliberately seeds nothing — overwriting a design-token reference with its resolved literal would be a silent, lossy edit). A `Custom…` escape hatch preserves the exact raw key/value form, so no uncurated property became unreachable. Live-verified end-to-end against the real compiled `gui-builder` CLI and a real `Card.jsx` + real `npm install`ed react: seeding read the real `#3366cc`/`24`+`px` out of the source, and all five control paths wrote real regenerated code to disk (`#ff8800`, `32px`, `3rem` via a unit-only change, `fontWeight: "bold"`, and `textTransform: "uppercase"` through Custom…). `desktop/`-only — `web/` has no Design screen at all (confirmed via grep), the same platform scope the Leo panel carries. |
| ~~Drag-and-drop on the visual canvas~~ | ✅ done | Shipped (task #279) — drag any element in the live canvas onto another to reparent it, routed through the exact same `Reparent` edit the "Move into" form already uses (including its real refusals for a root move or a move into a descendant, which surface as ordinary errors rather than being pre-checked twice). Deliberately pointer-based rather than HTML5 drag-and-drop: the canvas elements are rendered by the user's own React component and re-created on every re-render, so a `draggable="true"` attribute would need continual re-application, and HTML5 DnD can't be driven by ordinary synthetic mouse input either. A 5px movement threshold distinguishes a real drag from a plain click (on a freeform canvas a click routinely lands and releases on the same element), the drop target gets a live outline while dragging, and the click a drag generates is swallowed so a reparent never doubles as a stray selection change. All three edit paths in the Design screen (Apply button, component palette, canvas drag) were consolidated onto one shared `applyEditObject` so they can't drift in how they touch the live buffer. Live-verified end-to-end: a plain click still selects (`<p> #n2`) and does not reparent; a real drag of `<p>` onto `<h1>` nested it in the real source on disk without changing the selection. `desktop/`-only — `web/` has no Design screen. |
| ~~Component-library browser~~ | ✅ done | Shipped (task #278) — closes §75.90's own last named MVP gap. New `gui-builder/src/components.ts` scans a real project for `.jsx`/`.tsx` files and reports every exported React component (default and named, including `export { Card }` lists), skipping `node_modules`/`dist`/build output, depth-bounded, and skipping an unparseable file rather than failing the whole scan. The "is this a component?" test is a real, named heuristic — an exported binding starting with an uppercase letter, exactly the rule React itself enforces at the JSX call site — not deeper analysis. Each result carries the real relative module specifier an import in the currently-open file would need (`null` for a component declared in that same file). `ComponentInsert` gained `importFrom`/`importIsDefault`, and a new `ensureImport` adds as little as possible: nothing when the binding is already imported (a duplicate would be a real syntax error), a merged specifier into an existing import from the same module, and only otherwise a new statement placed after the existing import block. The Design screen gained a "Components (N)" palette, re-scanned on every open (never cached), with items disabled until a parent element is selected. Live-verified end-to-end against the real compiled CLI and a real multi-file fixture: the palette listed all 4 real components with correct origins, and inserting a cross-file **named** export produced `import { Badge } from "./shared/Button";` + `<Badge />` while a cross-file **default** export produced `import Banner from "./Banner";` + `<Banner />`, both confirmed in the real bytes on disk. 14 new tests (61 total in `gui-builder`). `desktop/`-only — `web/` has no Design screen. |
