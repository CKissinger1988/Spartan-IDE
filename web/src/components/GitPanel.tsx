 ...
 *
 * Real merge-conflict resolution UI (task #270), ported verbatim from
 * `desktop/`'s own copy: a "Merge" button on each non-current branch row
 * runs a real `git_merge_branch`; while a merge is genuinely in progress
 * a conflict panel lists every real conflicted file with "Take ours"/
...
export default function GitPanel({ client, root }: GitPanelProps): React.ReactElement {
  ...
  // Real merge-conflict resolution (task #270), mirroring desktop/'s own.
  ...
  // Real merge attempt (task #270), mirroring desktop/'s own.
  const mergeBranch = useCallback(
    ...
    [client, root, refresh, refreshMergeStatus]
  );

  // Real one-click conflict resolution, mirroring desktop/'s own.
  const resolveConflictWith = useCallback(
    ...
    [client, root, refreshMergeStatus]
  );
  ...
  const commitMerge = useCallback(() => {
    ...
    client
      ...
      .catch((e: Error) => setMergeError(e.message))
      .finally(() => setMergeBusy(false));
  }, [client, root, mergeCommitMessage, refresh, refreshMergeStatus]);

  // A real, destructive `git merge --abort` equivalent. Confirmed first.
  ...
  // Real "stage this one hunk", mirroring desktop/'s own.
  ...
  const cherryPick = useCallback(
    ...
    [client, root, refresh, showHistory]
  );
