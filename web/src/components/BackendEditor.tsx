...
/** One real source breakpoint, 1-indexed `line`. `condition` is a real
 * DAP conditional-breakpoint expression (stops only when truthy) and
 * `logMessage` turns it into a real logpoint (logs, never stops) -- both
 * optional; a bare `{ line }` is an ordinary breakpoint. Serialized into
 * `dap_launch`'s `breakpoints: [{line, condition?, logMessage?}]` param. */
...
/** A real, normalized call-hierarchy entry, ported verbatim from `desktop/`'s
 * own `CallerItem`. */
 ...
 * ported verbatim from `desktop/`'s own `CodeActionEnvelope` -- see that
 * file's own doc comment for the full real reasoning (deliberately typed
 * minimally since the full object, data and all, must be forwarded verbatim
 * to `codeAction/resolve`, never reconstructed). */
interface CodeActionEnvelope {
  title?: string;
  kind?: string;
}

/** The real display title of a raw code action, ported verbatim from
 * `desktop/`'s own `codeActionTitle`. */
 ...
 * shown next to its title in the quick-fix popup. Ported verbatim from
 * `desktop/`'s own `codeActionKindLabel`. */
...
/** A real, normalized, flattened LSP workspace symbol, ported verbatim
 * from `desktop/`'s own `WorkspaceSymbolItem` -- see that file's own doc
 * comment for the full real reasoning. `path` is decoded from the
 * backend's `uri` via `fileUriToPath` at normalization time. */
 ...
 * ported verbatim from `desktop/`'s own `extractWorkspaceSymbols` -- see
 * that file's own doc comment for the full real reasoning (the backend
 * already decoded the LSP 3.17 `location`-can-be-bare-`{uri}` wire shape;
 * a `null` result normalizes to an honest empty list, not an error). */
...
function joinLines(
  ...
  // With no real multi-line selection, join the current line with the next.
  const joinTo = lastLine > firstLine ? lastLine : firstLine + 1;
  if (joinTo > lines.length - 1) return null; // nothing below to join

  let joined = lines[firstLine];
  const seamOffset = joined.length; // caret lands at the first seam
 ...
 * own identical wiring -- see that file's own doc comment for the full
 * real reasoning (matches `search_project`'s own established plain-
 * substring, not regex, v1 scope). */
...
interface BackendEditorProps {
  ...
  /** Real, 1-indexed breakpoint specs for this file -- matches
   * `desktop/src/components/Editor.tsx`'s own convention exactly (the
   * gutter's own displayed line numbers, and the real DAP `breakpoints`
   * param `App.tsx` sends to `dap_launch` directly, no translation). Each
   * carries an optional `condition`/`logMessage`. */
  breakpoints?: BreakpointSpec[];
  /** Real click-to-toggle -- `App.tsx` owns the actual breakpoint set.
   * Always creates a plain breakpoint or removes whatever is there. */
...
export default function BackendEditor({
  ...
  // Real inline breakpoint condition/logpoint editor (right-click a gutter
  // line), ported verbatim from `desktop/src/components/Editor.tsx`.
  ...
  // Real call hierarchy (Shift+Alt+H incoming / Shift+Alt+O outgoing), ported
  ...
  /** Real LSP quick fixes (Alt+Enter), ported verbatim from `desktop/`'s
   * own identical wiring -- see that file's own doc comments for the full
   * real reasoning: the request carries the caller's full real diagnostics
   * for this file, the backend's `lsp_code_action` handler merged the
   * per-range requests for this position, and each action is kept verbatim
   * because picking one must forward it (data and all) to the real
  ...
  const triggerQuickFix = useCallback(
    ...
    [client, charWidth, lineHeightPx, file.docId, diagnostics]
  );

  /** Applies a resolved code action, ported verbatim from `desktop/`'s
   * own `applyResolvedCodeAction`: its `edit` is a real `WorkspaceEdit`
  ...
  // Real quick-fix handling -- both halves of the protocol, ported
  // verbatim from `desktop/`'s own identical effect (which mirrors the
  // rename effect above): `lsp_code_action_result` fills the popup,
  // `lsp_code_action_resolve_result` applies the picked action.
  useEffect(() => {
    const unsubscribe = client.onEvent((e) => {
      if (e.event === "lsp_code_action_result") {
        const d = e.data as {
          doc_id: number;
          line: number;
          character: number;
      ...
      } else if (e.event === "lsp_code_action_resolve_result") {
        const d = e.data as { doc_id: number; action: unknown };
        if (d.doc_id !== file.docId) return;
    ...
    return unsubscribe;
  }, [client, file.docId, applyResolvedCodeAction]);
  ...
  /** Real "Go to Symbol in Workspace" (Ctrl+T), ported verbatim from
   * `desktop/`'s own identical wiring -- see that file's own doc comment
   * for the full real reasoning (a searchable palette with a real focused
   * query `<input>`, debounced `workspace/symbol` requests, keyboard-
   * navigable results, the same stale-reply guard the other async
   * overlays use). Reached over `client.onEvent`/`client.call` instead of
   * `window.spartan`. */
  ...
  useEffect(() => {
    const unsubscribe = client.onEvent((e) => {
      if (e.event !== "lsp_workspace_symbol_result") return;
      const d = e.data as { doc_id: number; query: string; result: unknown };
   ...
   * `desktop/`'s own identical wiring -- see that file's own doc comment
   * for the full real reasoning (distinct from cross-file "Find in Files",
   * tasks #190-192; recomputes live on every query/case-sensitivity change
   * and every real edit). */
  ...
  /** A real bug, found live in `desktop/`'s own copy of this component --
   * see that file's own doc comment for the full account -- fixed
   * identically here: depend on whether the box is open, not the whole
   * object, so typing into the *replace* field doesn't keep yanking focus
   * back to the query field on every keystroke. */
  useEffect(() => {
    if (findState) findQueryInputRef.current?.focus();
  }, [Boolean(findState)]);

  /** A second real bug, also found live in `desktop/`'s own copy -- see
   * that file's own doc comment for the full account (a disabled
   * "Replace All" button drops focus entirely, leaving no focused
   * descendant of the find box for Escape to bubble through). Fixed
   * identically here with a real, global, only-while-open `window`
   * listener. */
  useEffect(() => {
    if (!findState) return;
  ...
  const findMatches = useMemo(() => {
    ...
    // itself) so live-typing while the find bar is open keeps the match
    // list and highlights in sync with the real edit.
  ...
  const replaceCurrentMatch = useCallback(() => {
    ...
    // `applyProgrammaticEdit` is deliberately not in the dep list -- see
    // the identical, already-explained reason in `desktop/`'s own copy.
  }, [findMatches]);
  ...
  const handleDefinitionClick = useCallback(
    ...
    [client, charWidth, lineHeightPx, file.docId, closeWorkspaceSymbols]
  );
  ...
  const applyProgrammaticEdit = useCallback(
    (el: HTMLTextAreaElement, newContent: string, selStart: number, selEnd: number) => {
      ...
      const oldLength = [...prevContentRef.current].length;
      // Snippet tab-stop tracking: if a snippet session is active, shift its
      // recorded stop offsets by this edit's delta (computed against the
      // pre-edit content) so Tab still lands correctly after the user types
      // at a stop. Done before prevContentRef is overwritten below.
  ...
  const handleKeyDown = useCallback(
    (e: React.KeyboardEvent<HTMLTextAreaElement>) => {
      ...
      // Escape directly); ported verbatim from `desktop/`'s own branch.
      ...
      // Real quick-fix dismissal (Escape), ported verbatim from
      // `desktop/`'s own identical branch.
      ...
      // Real, manual call-hierarchy triggers (Shift+Alt+H incoming /
      // Shift+Alt+O outgoing), ported verbatim.
      ...
      // Real quick-fix trigger (Alt+Enter), ported verbatim from
      // `desktop/`'s own identical branch -- see that file's own doc
      // comment for why `e.altKey` (not `e.code`/`e.key` pair matching)
      // is what makes it Alt+Enter.
      ...
      // ported verbatim from `desktop/`'s own identical branches -- see
      // that file's own doc comment for the full real reasoning.
      ...
      // Real "Go to Symbol in Workspace" trigger (Ctrl+T), ported verbatim
      // from `desktop/`'s own identical branch -- the palette's own input
      // handles Enter/Escape/Up/Down, so this never fires again while open.
  ...
  return (
    <div className="editor-root">
              ...
              {lineDiags && lineDiags.length > 0 && (
                <span
                  ...
                  onClick={(e) => {
                    ...
                    // breakpoint-toggle click -- ported verbatim from
                    // `desktop/`'s own identical gutter wiring.
                    e.stopPropagation();
                    e.preventDefault();
      ...
      {breakpointEdit && onEditBreakpoint && (
        ...
        >
          <div className="editor-breakpoint-edit-title">
            Breakpoint · line {breakpointEdit.line}
          </div>
