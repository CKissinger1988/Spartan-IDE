...
/// Real, live `textDocument/semanticTokens/full` (semantic highlighting) --
/// the direct sibling of `lsp_document_symbol` above: identical
/// never-blocks-the-caller shape, and like `lsp_document_symbol` it takes no
/// `line`/`character` (a semantic-token request covers the whole document,
/// not one cursor position). One deliberate difference from every other
/// `lsp_*` query handler here: it does **not** unwrap a `result` field off a
/// raw JSON-RPC envelope -- `LspSession::request_semantic_tokens` already
/// returns the decoded, legend-resolved `{line, character, length,
/// token_type, modifiers}` span array (the LSP client crate decodes the flat
/// `u32` wire encoding against the server's own legend, since that encoding
/// is meaningless without it), so the event's `result` *is* the payload.
fn lsp_semantic_tokens(
    ...
    thread::spawn(move || {
        let result = session.request_semantic_tokens();
        let event = Event {
            event: "lsp_semantic_tokens_result".to_string(),
            data: serde_json::json!({
                "doc_id": doc_id,
                "result": result,
            }),
...
"#;
    ...
    #[test]
    fn lsp_semantic_tokens_on_a_real_unopened_doc_id_errors_honestly() {
        let state = new_state();
        let resp = call(
            &state,
            1,
    ...
    #[test]
    fn lsp_semantic_tokens_on_a_real_open_synthetic_file_with_no_lsp_session_errors_honestly() {
        // The direct sibling of `lsp_hover`'s/`lsp_completion`'s/
        // `lsp_definition`'s/`lsp_signature_help`'s/`lsp_references`'s/
        // `lsp_rename`'s/`lsp_document_symbol`'s/`lsp_document_highlight`'s
        // own identical tests above -- same real, honest error path, same
        // "unrecognized extension never gets a real LSP session" cause.
        let dir = std::env::temp_dir().join(format!(
            "spartan-backend-lsp-semantic-tokens-no-session-{}-{:?}",
            std::process::id(),
            std::thread::current().id()
        ));
        let _ = std::fs::remove_dir_all(&dir);
