//! Real Cargo build-system integration for DAP, copied (not imported) from
//! `crates/spartan-editor-core/src/build.rs` -- same second-promotion
//! rationale as `client.rs`. Runs a real `cargo build --message-format=json`
//! and parses its streamed JSON output for the resulting binary's path,
//! using cargo's own documented message format.
...
pub enum BuildResult {
    ...
    Success(PathBuf),
    /// Real, rendered compiler diagnostics, or a single explanatory line
    /// if cargo itself couldn't be spawned or produced no diagnostics.
    Failure(Vec<String>),
}
...
/// a request-handling one, since a real build can take anywhere from under
/// a second to minutes. Only ever looks for a `bin` target's `executable`.
