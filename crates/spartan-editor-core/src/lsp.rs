//! Promoted from `spikes/lsp-spike/src/lib.rs` (§39.2, §47.5-§47.8) verbatim
//! -- a real, in-house LSP client speaking real JSON-RPC 2.0 over stdio,
//! proven against two independent real servers (`rust-analyzer`,
//! `pyright-langserver`) by that spike's own tests. No logic changes were
//! needed to become real product code; `same_file_uri` is promoted as
//! `pub(crate)` rather than private, since `lsp_session.rs` (this crate's
//! new orchestration layer, not present in the spike) needs it too.
...
impl LspClient {
    ...
    /// object `{}` where the spec calls for no params at all).
    pub fn request(
        &mut self,
        method: &str,
        params: Option<Value>,
    ...
    /// on open, then real ones once indexing/analysis finishes). Note this
    /// can never observe diagnostics *clearing back to empty* -- by design,
    /// it only ever resolves on a non-empty array. `lsp_session.rs` uses
    /// `wait_notification` directly instead for that case.
    ///
    ...
    /// Graceful shutdown: `shutdown` request (no params), then `exit`
    /// notification, with a bounded kill-fallback that never trusts the
    /// subprocess's own shutdown (§4.5). Blocks for up to ~7s worst case
    /// (5s shutdown-request timeout + 2s process-reap poll) -- callers on a
    /// UI thread should surface that this may take a moment, not call it
    /// silently.
...
/// real language server to test: it's a pure scheduling policy over
/// wall-clock time.
